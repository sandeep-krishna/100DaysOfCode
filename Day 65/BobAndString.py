...
for i in xrange(num_test_cases):
    counter = 0
    correct_dict = get_dict(raw_input())
    wrong_dict = get_dict(raw_input())
    for key, val in wrong_dict.iteritems():
        if key in correct_dict:
            counter += abs(correct_dict[key] - val)
        else:
            counter += val

    for key, val in correct_dict.iteritems():
